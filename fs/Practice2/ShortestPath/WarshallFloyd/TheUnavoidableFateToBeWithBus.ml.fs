...
let createGraph = fun (n, abts) ->
  (
    Array.init n (fun _ -> Array.create n (System.Int32.MaxValue / 2))
  )
  |> fun graph ->
    (
      seq { 0..graph.Length - 1 }
      |> Seq.iter (fun i -> graph.[i].[i] <- 0),
      abts
      |> Seq.iter (fun (a, b, t) ->
        (
          (graph.[a - 1].[b - 1] <- t),
...
let minimumWorstTime = (Seq.map Seq.max) >> Seq.min
...
// https://atcoder.jp/contests/abc012/submissions/22985685
